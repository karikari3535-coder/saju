...
function extractDate(
  ...
  // 패턴 B: 1990.05.15 / 1990-5-15 / 1990/05/15 / 1969~10~21 (물결표 구분 포함)
  //   구분자에 틸드(~)·전각 물결(～·〜)도 허용한다.
  //   (예: "1969~10~21 아침 10시" → 연1969 월10 일21)
  ...
  //   구분자에 틸드(~)·전각 물결(～·〜)도 허용한다. (예: "69~10~21")
  //   뒤 lookahead: 같은 구분자로 4번째 숫자 묶음이 이어지면 거부(범위 등). 단, "10시"처럼 시각이 붙는 건 허용.
  m = text.match(/(?<!\d)(\d{2})\s*[.\-\/~～〜]\s*(\d{1,2})\s*[.\-\/~～〜]\s*(\d{1,2})(?!\s*[.\-\/~～〜]\s*\d)(?!\d)/)
  if (m) {
    const yy = +m[1]
    const year = yy <= 25 ? 2000 + yy : 1900 + yy
    ambiguity.push(`연도를 2자리(${m[1]})로 적어 ${year}년으로 추정했어요. 확인해 주세요.`)
